...
class EffortDetector(nn.Module):
    ...
    def build_backbone(self, config):
        ...
        # 使用 CLIP-ViT-L/14 的配置直接初始化模型结构（不加载预训练权重）
        # 配置参数来自 openai/clip-vit-large-patch14
        ...
        # 初始化 vision_model（随机权重，后续会被 effort_clip_L14.pth 覆盖）
        vision_model = CLIPVisionModel(vision_config)

        print("✅ CLIP Vision Model 结构初始化完成（等待加载 Effort 检测器权重）")
        # ========== 修复结束 ==========

        # Apply SVD to self_attn layers only
        # ViT-L/14 224*224: 1024-1
        vision_model = apply_svd_residual_to_self_attn(vision_model, r=1024 - 1)

        # for name, param in vision_model.named_parameters():
            # print('{}: {}'.format(name, param.requires_grad))

        num_param = sum(p.numel() for p in vision_model.parameters() if p.requires_grad)
        num_total_param = sum(p.numel() for p in vision_model.parameters())
